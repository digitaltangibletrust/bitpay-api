...
function validate(invoice, cb) {
  ...
  if(!price) return cb('Field price is mandatory');
  if(!currency) return cb('Field currency is mandatory');
  if(!validator.isFloat(price)) return cb('Field price must be numeric');
  if(_.indexOf(currencyCodes, currency) === -1) return cb('Invalid currency code: ' + currency + '. Must be one of ' + currencyCodes.join(','));  // quicker look-ups with a hash of cc's at some point

  // Begin Optional fields
  if(notificationURL && !validator.isURL(notificationURL)) return cb('Invalid notificationURL: ' + notificationURL);
  if(redirectURL && !validator.isURL(redirectURL)) return cb('Invalid redirectURL: ' + redirectURL);
  if(notificationEmail && !validator.isEmail(notificationEmail)) return cb('Invalid notificationEmail: ' + notificationEmail);
  if(transactionSpeed && _.indexOf(validTransactionSpeeds, transactionSpeed) === -1) return cb('Invalid transactionSpeed: ' + transactionSpeed + '. Must be one of ' + validTransactionSpeeds.join(','));
  if(!_.isUndefined(fullNotifications) && !_.isBoolean(fullNotifications)) return cb('Invalid fullNotifications: ' + fullNotifications + '. Must be boolean');
  if(!_.isUndefined(physical) && !_.isBoolean(physical)) return cb('Invalid physical: ' + physical + '. Must be boolean');
  ...
  for(var i in string100s) {
    ...
    if(value && (!_.isString(value) || value.length > 100)) return cb('Field ' + field + ' invalid. String should be 100 characters or less')
  }
